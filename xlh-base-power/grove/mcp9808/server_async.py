...
from mcp9808 import helper  # type: ignore[import-not-found]

_logger = logging.getLogger(__file__)
_logger.setLevel(logging.ERROR)


def setup_server(description=None, context=None, port: int | None = None):
    """Run server setup."""
    args = helper.get_commandline(server=True, description=description, cmdline=None)
    if port:
        args.port = port
    if context:
