...
void setup()
{
    ...
    Serial1.printf("getLEDMode :%d\n", device.getLEDShowMode());
    delay(1000);
    Serial1.printf("Switch Status:%d\n", device.getSwitchStatus());
    delay(1000);
    for (uint8_t i = 0; i < 8; i++) {
        uint8_t switchStatus8Bytes = device.getSwitchStatus(i);
        Serial1.print("Switch Status Byte ");
        Serial1.print(i);
        Serial1.print(": ");
        Serial1.println(switchStatus8Bytes);
    }
    delay(1000);

    device.setLEDShowMode(BYTE_LED_USER_DEFINED);
    Serial1.println("Set LED show self define.");
    delay(1000);
    ...
    Serial1.printf("New LED Brightness:: %02X\n", device.getLEDBrightness());
    delay(1000);
    // RGB
    for (int i = 0; i <= 8; i++) {
        ...
        Serial1.printf("Set RGB to %06X\n", (unsigned int)colors[i]);
    }
    delay(1000);

    for (int i = 0; i < 8; i++) {
        // Serial1.printf("Retrieved  RGB: %06X\n",device.getRGB888(i));
        Serial1.printf("Retrieved  RGB: %06X\n", device.getRGB233(i));
    }
    ...
    delay(1000);
    Serial1.printf("getIRQEnable: %02x\n", device.getIRQEnable());
    delay(1000);
    Serial1.printf("Firmware Version: %02x\n", device.getFirmwareVersion());
    delay(1000);

    Serial1.printf("read I2C Address:%02X\n", device.getI2CAddress());
    delay(1000);

    device.setI2CAddress(0x12);
    delay(1000);
    Serial1.printf("new I2C Address: %02x\n", device.getI2CAddress());
    delay(2000);
    device.setI2CAddress(buttonId);
    delay(1000);

    device.setLEDShowMode(BYTE_LED_MODE_DEFAULT);
    Serial1.println("Set LED show sys define.");
    delay(1000);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Output the hexadecimal value of the current color
        Serial1.printf("Set Switch Off RGB to %06X\n", (unsigned int)colors[i]);
    }
    delay(1000);
    for (int i = 0; i < 8; i++) {
        device.setSwitchOnRGB888(i, colors[9 - i]);
        // Output the hexadecimal value of the current color
        Serial1.printf("Set Switch On RGB to %06X\n", (unsigned int)colors[i]);
    }
    delay(1000);
    device.setFlashWriteBack();
}
