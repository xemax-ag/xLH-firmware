...
void CAN_OPEN::rx_pdo_1(twai_message_t *msg_rx)
{
  ...
  if ((msg_rx->identifier == this->pdo_rx_1_id) && (msg_rx->data_length_code == 4))
  {
    ...
    brightness = this->in.abyRxData1[3];
    ...
    chain2.in.rgbLed[0] = (this->in.abyRxData1[1] & 0b00110000) >> 4;
    chain2.in.rgbLed[1] = (this->in.abyRxData1[1] & 0b11000000) >> 6;

    chain2.in.rgbLed[2] = (this->in.abyRxData1[2] & 0b00000011);
...
void CAN_OPEN::tx_pdo_1(void)
{
  ...
  portENTER_CRITICAL_ISR(&chain2Mux);
    this->out.abyTxData1[1] = chain2.out.value8Bit[0];
  portEXIT_CRITICAL_ISR(&chain2Mux);
