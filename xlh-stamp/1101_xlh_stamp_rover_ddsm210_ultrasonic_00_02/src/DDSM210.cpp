...
void DDSM210::loop(void)
{
	portENTER_CRITICAL_ISR(&ddsm210Mux);
		memcpy(&this->inDdsm210, &can_open.inDdsm210, sizeof(inDdsm210));
  ...
  delay(1);
  dc.clear_ddsm_buffer();
  dc.ddsm_ctrl(1, this->inDdsm210.iSetSpeed, this->inDdsm210.bySetAccTime);
  ...
  this->outDdsm210.iSpeed = dc.speed_data;
  this->outDdsm210.iCurrent = dc.current;
  this->outDdsm210.byAccTime = dc.acceleration_time;
  this->outDdsm210.byTemperature = dc.temperature;
  ...
  this->outDdsm210.uiErrorCode = (uint16_t(this->byErrorCode1) << 8) + uint16_t(this->byErrorCode2);
  ...
  this->outDdsm210.diPosition = 0;// this->diPosition;
  ...
  portENTER_CRITICAL_ISR(&ddsm210Mux);
		memcpy(&can_open.outDdsm210, &this->outDdsm210, sizeof(this->outDdsm210));
	portEXIT_CRITICAL_ISR(&ddsm210Mux);
}
